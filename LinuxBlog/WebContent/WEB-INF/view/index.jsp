...
<body>
	...
	<div id="content">
		<div id="c_center">
			<section>
				<div class="c_header">NAJNOWSZE WPISY</div>
				<c:forEach var="thread" items="${threads}">
					<div class="thread">
						<img src="imageServlet?type=thread&id=${thread.id}" />
						<div class="thread_topic">
							<c:url var="aboutUserUrl" scope="page"
								value="user?id=${thread.user.id }"></c:url>
							<a href="${pageScope.aboutUserUrl }"><i
								class="demo-icon icon-user"
								style="color: #000033; text-decoration: none"></i><span
								style="font-size: 12px; color: #000033">${thread.user.login }</span></a>
							<span
								style="font-size: 12px; border-right: 1px solid black; margin-left: 5px; margin-right: 10px; padding-right: 10px"><fmt:formatDate
									value="${thread.addDate}" dateStyle="long" timeStyle="short"
									type="both" /> </span> ${thread.topic }
						</div>
						<div class="thread_text">${thread.content }<a
								href="thread?id=${thread.id}">(więcej)</a>
						</div>
						<a href="thread?id=${thread.id}"><i class="demo-icon icon-login" style="color: #000"></i></a>
					</div>
				</c:forEach>
			</section>
			<div class="pages">
				<c:forEach begin="1" end="${allThreads}" varStatus="loop" var="pg">
					<a href="home?p=${pg}">&nbsp;${pg}&nbsp;</a>
				</c:forEach>
			</div>
		</div>
		<div id="c_right">
