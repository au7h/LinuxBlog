...
<body>
	...
	<div id="content">
		<div id="c_center">
			<section>
				<div class="c_header">${sessionScope.user.login }-edytujprofil</div>
				<div class="about_user">
					<form action="" method="post" enctype="multipart/form-data">
						<p>Avatar:</p>
						<img src="imageServlet?type=user&id=${sessionScope.user.id}" /> <input
							type="file" name="photo" id="file" style="border: none" />
						<p>Login:</p>
						<input name="login" value="${sessionScope.user.login}" />
						<p>Hasło:</p>
						<input type="password" name="password" />
						<p>Imię:</p>
						<input name="firstName" value="${sessionScope.user.name}" />
						<p>Nazwisko:</p>
						<input name="lastName" value="${sessionScope.user.surname}" />
						<fmt:formatDate pattern="yyyy-MM-dd" var="parsedDate"
							scope="session" value="${sessionScope.user.birthDate}" />
						<p>Data urodzenia:</p>
						<input type="date" name="birthDate"
							value="${sessionScope.parsedDate}" />
						<p>Adres zamieszkania:</p>
						<input name="address" value="${sessionScope.user.address}" />
						<p>Adres e-mail:</p>
						<input name="email" value="${sessionScope.user.email}" /><br />
